...
{
    public class UdpChannel : Runnable, IChannel
    {
        private static readonly ILogger logger = Log.ForContext<UdpChannel>();

        private readonly byte[] buffer = new byte[4 * 1024];

        private readonly TcpClient client;

        public UdpChannel(TcpClient client)
        {
            this.client = client;

            var socket = client.Client;
            var ipEndPoint = (IPEndPoint)socket.RemoteEndPoint;
            RemoteEndPoint = new UdpEndPoint(ipEndPoint.Address, ipEndPoint.Port);
        }
